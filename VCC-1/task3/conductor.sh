...
run()
{
    ...
    unshare --uts --pid --net --mount --ipc --fork --kill-child chroot "$CONTAINERDIR/$NAME/rootfs" /bin/bash -c "
    chmod 755 /
...
exec()
{
    ...
    # running within the container
     nsenter --target "$CONTAINER_INIT_PID" --all  -w -r  /bin/bash -c "exec $EXEC_CMD_ARGS"
    # nsenter --target "$CONTAINER_INIT_PID" --all  -w -r  -- "$$EXEC_CMD_ARGS"
